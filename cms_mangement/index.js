...
// @description  cms内部管理
// @author       zzailianlian
// @updateURL    https://raw.githubusercontents.com/zzall/temperMonkey/master/cms_mangement/index.js
...
;(function () {
  ...
  var onloaded = function () {
    ...
    const wrapperToolsContainer = $(
      '<div id="wrapperToolsContainer">更新的页面id：<input type="number" style="margin-bottom:8px;"/></div>'
    )
    ...
    syncConfig.on('click', () => {
      ...
      const cmsPageFrameJq = getFrameJQ(
        'mainIframe_tabli_' + getIdFromTab(GM_getValue(CMS_PAGE_TAB_ID))
      )
      // 编辑页面
      cmsPageFrameJq
        .find('tr')
        .find('td:contains(' + MODULE_ID + ')')
        .parent()
        .find('button:contains("编辑")')
        .click()
      ...
      loop(
        ...
        function () {
          ...
          const cmsPageModalFrameJq = getFrameJQ(
            'mainIframe_modifyPage' + MODULE_ID
          )
          loop(
            function () {
              // 副标题存在数据，说明【编辑操作】掉接口回填成功
              return !!$(getFrameJQ('mainIframe_modifyPage' + MODULE_ID))
                .find('input[placeholder="如：APP新客活动"]')
                .last()
                .attr('value')
            },
            function () {
              // 下一步
              $(getFrameJQ('mainIframe_modifyPage' + MODULE_ID))
                .find('input[value="下一步"]')
                .last()
                .click()
              loop(
                function () {
                  ...
                  loop(
                    function () {
                      ...
                      return !!$(
                        getFrameJQ(
                          'mainIframe_tabli_' +
                            getIdFromTab(GM_getValue(CMS_PAGE_TAB_ID))
                        )
                      )
                        .find('tr')
                        .find('td:contains(' + MODULE_ID + ')')
                        .parent()
                        .find('button:contains("同步线上")').length
                    },
                    function () {
                      // 同步线上
                      $(
